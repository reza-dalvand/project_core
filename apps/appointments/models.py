...
class Appointment(BaseModel):
    ...
    is_trust_based = models.BooleanField(
        "اعتماد به سالن",
        default=False,
    )
    ...
    def __str__(self):
        return (
            ...
            f"{self.service.name} ({self.date_key})"
        )

    def save(self, *args, **kwargs):
        self.date_key = (
            f"{self.jy}/{self.jm:02d}/{self.jd:02d}"
        )
